...
Let's assume that we'd like to map _Person_ to _PersonDto_ (and vice-versa); there are a few interesting points about these two classes. First, _Person_ has a 'name' property which is of type _Name_, while _PersonDto_ contains the name data in two string fields 'firstName' and 'lastName'. 
