...
If you're using IDEA Intellij as your IDE, you have to open the "Project structure..." menu, add a Java library and point to the generated folder.

If you're using any other IDE, the procedure should be similar. Note that although Orika is leveraging the Eclipse Jdt core functions to format and compile the code, it resolves these from it's own dependencies, so if using Maven, there should be few differences.
